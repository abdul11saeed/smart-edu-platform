...
export default function ResetPassword() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setMessage('تم إرسال رابط إعادة تعيين كلمة المرور إلى بريدك الإلكتروني. يرجى التحقق من صندوق الوارد أو الرسائل غير المرغوب فيها.');
            console.log('Password reset email sent successfully');
        } catch (err: any) {
            console.error('Password reset failed:', err);
            setError(err.message || 'حدث خطأ، يرجى التأكد من البريد الإلكتروني');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    return (
        <div className="flex items-center justify-center py-12 px-4">
            <div className="max-w-md w-full space-y-6 p-8 bg-white/80 dark:bg-[#0F172A]/80 backdrop-blur-xl rounded-2xl shadow-lg shadow-[#1E40AF]/5 border border-slate-200/80 dark:border-blue-400/20">
                <h2 className="text-center text-3xl font-bold bg-gradient-to-l from-[#1E40AF] to-[#0F172A] dark:from-blue-300 dark:to-blue-100 bg-clip-text text-transparent">إعادة تعيين كلمة المرور</h2>
                <p className="text-center text-slate-600 dark:text-blue-200/70 text-sm">
                    أدخل بريدك الإلكتروني المسجل في النظام. إذا كان لديك حساب مسجل بالبريد الإلكتروني وكلمة المرور،
