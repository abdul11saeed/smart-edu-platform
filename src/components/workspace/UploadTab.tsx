...
const UploadTab = ({ courseId }: { courseId: string }) => {
    ...
    return (
        <div dir={i18n.language === 'ar' ? 'rtl' : 'ltr'} className="max-w-2xl mx-auto">
            <div className="glass-card-modern p-6 sm:p-8">
                <div className="flex items-center gap-3 mb-2">
                    ...
                    <h2 className="text-xl font-bold bg-gradient-to-l from-primary-700 to-secondary-600 bg-clip-text text-transparent">{t('upload.title')}</h2>
                </div>
                <p className="text-sm text-brown-500 mb-6">
                    {t('upload.courseAutoSelected')}
                </p>
                <FileUploadForm
                    courseId={courseId}
