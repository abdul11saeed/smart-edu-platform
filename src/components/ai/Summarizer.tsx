...
const createLocalSummary = (text: string, t: (key: string) => string) => {
    const sentences = text.split(/[.!?。\n]+/).map(s => s.trim()).filter(Boolean);
    const intro = sentences.slice(0, 2).join(' ');
    const closing = sentences.length > 2 ? ` — ${t('ai.summarizer.keyPoints')}: ${sentences.slice(2, 5).join(' ')}` : '';
...
const Summarizer = ({ fileName, fileContent, file, onClose }: SummarizerProps) => {
    ...
    const handleExport = () => {
        ...
        let content = `${t('ai.summarizer.summaryTitle')}\n`;
        content += `${'='.repeat(50)}\n\n`;
        content += `${summaryData.summary}\n\n`;
        if (summaryData.keyPoints.length > 0) {
            content += `${t('ai.summarizer.keyPoints')}\n`;
        ...
        a.href = url;
        a.download = `summary_${fileName || 'content'}_${Date.now()}.txt`;
    ...
    return (
      <div dir={i18n.language === 'ar' ? 'rtl' : 'ltr'} className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 max-w-4xl mx-auto text-gray-900 dark:text-gray-100">
        ...
        {fileName && (
          <p className="text-sm text-gray-800 dark:text-gray-200 mb-4">{t('ai.summarizer.fileName', { name: fileName })}</p>
        )}
        ...
        {(!fileContent?.trim() || ['[ملف', '[File'].some(prefix => fileContent?.startsWith(prefix))) && (
          <div className="rounded-lg border border-dashed border-primary-200 dark:border-primary-700 bg-primary-50 dark:bg-primary-900/30 p-4 text-center text-gray-800 dark:text-gray-200 mb-6">
            <p className="font-medium mb-2">{t('ai.summarizer.notExtracted')}</p>
            <p className="text-sm text-gray-700 dark:text-gray-300 mb-3">{t('ai.summarizer.pasteText')}</p>
            <textarea
              value={manualText}
              onChange={(e) => setManualText(e.target.value)}
              placeholder={t('ai.summarizer.pastePlaceholder')}
        ...
        <div className="mb-6">
          ...
          >
            {isSummarizing ? t('ai.summarizer.generating') : t('ai.summarizer.generateButton')}
          </button>
        </div>
        ...
        {summaryData && (
          <div className="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100">{t('ai.summarizer.summaryTitle')}</h3>
              <button
            ...
            {summaryData.keyPoints && summaryData.keyPoints.length > 0 && (
              <div className="mb-4">
                <h4 className="font-medium text-gray-900 dark:text-gray-100 mb-2">{t('ai.summarizer.keyPoints')}</h4>
            ...
            <div className="mt-4 flex items-center text-sm text-gray-500 dark:text-gray-400">
              <span>{t('ai.summarizer.generatedAt', { time: new Date().toLocaleTimeString(i18n.language === 'ar' ? 'ar-SA' : 'en-US') })}</span>
            </div>
          </div>
        )}
        ...
        {!summaryData && !isSummarizing && (
          <div className="text-center py-12 text-gray-800 dark:text-gray-300">
            ...
            <p className="font-medium text-gray-700 dark:text-gray-200">{t('ai.summarizer.selectFile')}</p>
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-2">{t('ai.summarizer.canPasteManually')}</p>
          </div>
        )}
      </div>
    );
